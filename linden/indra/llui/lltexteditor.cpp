...
#include "../newview/llfilepicker.h"
#include "../newview/llviewerwindow.h"
#include <queue>
...
void LLTextEditor::loadFromFile()
{
	llinfos << "load from file (" << mMaxTextByteLength << ")" << llendl;
	
	if( !isPristine()) {
		llwarns << "Script has unsaved changes, loading from disk aborted." << llendl;
		LLStringBase<char>::format_map_t args;
		args["[REASON]"] = std::string("Existing script has unsaved changes. You must save this script before loading from disk.");
	...
	LLString filename = file_picker.getFirstFile();
	FILE* file = LLFile::fopen(filename.c_str(), "rb");		/*Flawfinder: ignore*/
	if(file)
	{
		// read in the whole file if it's not too big
		fseek(file, 0L, SEEK_END);
		long file_length = ftell(file);
		if( file_length > mMaxTextByteLength ) {
			fclose( file );
			llwarns << "Loaded file is too big, aborted. (" << file_length << "/" << mMaxTextByteLength << ")" << llendl;
			LLStringBase<char>::format_map_t args;
			args["[REASON]"] = std::string("The file you are loading is too big. Loading aborted.");
			gViewerWindow->alertXml("LoadDiskScriptFailReason", args);		
			return;
		}
...
BOOL LLTextEditor::canLoadFromFile()
{
	if( !mReadOnly )
		return TRUE;
	return FALSE;
...
void LLTextEditor::saveToFile()
{
	llinfos << "save to file" << llendl;
	LLFilePicker& file_picker = LLFilePicker::instance();
	if( !file_picker.getSaveFile( LLFilePicker::FFSAVE_ALL ) )
	{
		return;
	}
	...
	if( !fp ) 
	{
		llwarns << "Unable to write: " << filename << llendl;
		return;
	}
	
	LLString utf8text = getText();
	fputs( utf8text.c_str(), fp );
	fclose( fp );
	fp = NULL;
}

BOOL LLTextEditor::canSaveToFile()
{
	if( !mReadOnly )
		return TRUE;
	return FALSE;	
