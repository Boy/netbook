...
U64											LLKeyframeDataCache::sTimeLastCacheCleanup;
...
void LLKeyframeDataCache::cleanupCache(S32 maxage)
{
	...
	if (0 == sTimeLastCacheCleanup) // first call
	{
		sTimeLastCacheCleanup = now;
		return;
	}

	// last cleanup run longer than a minute ago, or explicite call for cleanup (maxage!=0)
	if (now-sTimeLastCacheCleanup > 60 || maxage != 0) 
	{
		S32 count = sKeyframeDataMap.size();
		S32 deleted = 0;

		if (0 == maxage) {
			// max age dynamic depending on cache fill: : 30 min - 5 min for num entries 0 - 250+
			maxage = 60 * (S32)(30.f - llclamp((F32)count/10, 0.f, 25.f));
		}

		llinfos << "has " << count << " entries (current max idle age before flush " 
				<< maxage/60 << ":" << maxage%60  << " minutes)" 
				<< llendl;
		...
		if (deleted>0) 
		{
			llinfos << "some entries deleted: total now " << sKeyframeDataMap.size() << " (-" << deleted << ") entries " << llendl;
		}
