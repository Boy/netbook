	...
	public:
		...
		virtual void completed(bool success)
		{
			if(success && (mFormattedImage.notNull()) && mImageSize>0)
			{
				...
				llinfos << "Saving to "<<(primbackup::getInstance()->getfolder()+"//"+uuid_str)<<llendl;			
				if(!mFormattedImage->save(primbackup::getInstance()->getfolder()+"//"+uuid_str))
				{
...
:	LLFloater("Prim Import Floater", "FloaterPrimImportRect", "")
{
	gUICtrlFactory->buildFloater(this, "floater_prim_import.xml");
...
void primbackup::updateexportnumbers()
{
	...
	LLUICtrl * ctrl = (LLUICtrl*)this->getChildByName("name_label", TRUE);	
...
{
	std::size_t offset = filepath.find_last_of(gDirUtilp->getDirDelimiter());
...
void primbackup::pre_export_object()
{
	...
	file_name = file_picker.getFirstFile();
	folder = getDirName(file_name);
		...
		case EXPORT_STRUCTURE:
		{
			struct ff : public LLSelectedObjectFunctor
			{
				virtual bool apply(LLViewerObject* object)
				{
					...
					LLSD prim_llsd=primbackup::getInstance()->prims_to_llsd(object);				
					LLSD stuff;
			...
			gSelectMgr->getSelection()->applyToRootObjects(&func,false);
			gSelectMgr->getSelection()->unref();
		...
		case EXPORT_LLSD:
			{
				...
				llofstream export_file(primbackup::getInstance()->file_name.c_str());
...
LLSD primbackup::prims_to_llsd(LLViewerObject *linkset)
{
	...
	for (U32 child_num = 0; child_num <= linkset->mChildList.size(); child_num++)
	{
		if (child_num == linkset->mChildList.size())
		{
			object = linkset;
		}
		else
		{
			object = linkset->mChildList[child_num];
		}
		...
		LLSelectNode* node = gSelectMgr->getSelection()->findNode(object);
		if (node)
		{
...
void primbackup::import_object(bool upload)
{
	...
	llifstream import_file(file_name.c_str());
...
void primbackup::xmltoprim(LLSD prim_llsd,LLViewerObject * object)
{
	...
	{
		gSelectMgr->selectionSetObjectName(prim_llsd["name"].asString());
	}

	if(prim_llsd.has("description"))
	{
		gSelectMgr->selectionSetObjectDescription(prim_llsd["description"].asString());
	}
...
void primbackup::prim_update(LLViewerObject* object)
{
	...
	if(prim_import_iter==this_group.endMap())
	{
		...
		if(toselect.size()>1)
		{
			...
			//Now link
			gSelectMgr->deselectAll();
			gSelectMgr->selectObjectAndFamily(toselect,true);
			gSelectMgr->sendLink();
...
void primbackup::upload_next_asset()
{
	...
	current_asset=id;
	char struid[UUID_STR_LENGTH];		/*Flawfinder: ignore*/
	id.toString(struid);
	std::string filename(struid);
