...
LLPointer<LLViewerImage> LLWorldMapView::sEventAdultImage = NULL;
...
LLPointer<LLViewerImage> LLWorldMapView::sForSaleAdultImage = NULL;
...
void LLWorldMapView::initClass()
{
	...
	sEventAdultImage = gImageList.getImage(image_id, MIPMAP_FALSE, TRUE);
	...
	sForSaleAdultImage = gImageList.getImage(image_id, MIPMAP_FALSE, TRUE);
}
...
// 		LLTextureView::addDebugImage(simimage);
	...
	if (gSavedSettings.getBOOL("MapShowLandForSale"))
	{
		...
		// We're showing normal land and adult land in the same UI; you don't
		// get a choice about which ones you want. If you're currently asking
		// for adult content and land you'll get the adult land.
		if (gAgent.canAccessAdult())
		{
			drawGenericItems(gWorldMap->mLandForSaleAdult, sForSaleAdultImage);
		}
	}
...
void LLWorldMapView::drawEvents()
{
	...
	if (show_adult)
	{
		for (e = gWorldMap->mAdultEvents.begin(); e != gWorldMap->mAdultEvents.end(); ++e)
		{
			if (!e->mSelected)
			{
								...
								F32 dot_radius)
{
	...
	if (UseOldTrackingDots || relative_z == 16000.f)
	{
		F32 left  = 	x_pixels - dot_radius;
		F32 right = 	x_pixels + dot_radius;
		F32 center =	(left + right) * 0.5f;
								 ...
								 LLUUID* id)
{
	...
	*hit_type = 0; // hit nothing
	...
	for (it = gWorldMap->mAdultEvents.begin(); it != gWorldMap->mAdultEvents.end(); ++it)
	{
		(*it).mSelected = FALSE;
	}
	...
	if (gSavedSettings.getBOOL("MapShowLandForSale"))
	{
		...
		for (it = gWorldMap->mLandForSaleAdult.begin(); it != gWorldMap->mLandForSaleAdult.end(); ++it)
		{
			LLItemInfo& land = *it;
