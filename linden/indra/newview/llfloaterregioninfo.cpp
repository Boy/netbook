...
BOOL LLPanelRegionGeneralInfo::sendUpdate()
{
	...
	else
	{
		...
		snprintf(buffer, MAX_STRING, "%d", childGetValue("access_combo").asInteger());		/* Flawfinder: ignore */
		strings.push_back(strings_t::value_type(buffer));
...
bool LLPanelEstateInfo::refreshFromRegion(LLViewerRegion* region)
{
	...
	LLTextBox* region_maturity = LLUICtrlFactory::getTextBoxByName(this, "region_maturity_text");
	if (region_maturity)
	{
		std::string maturity = region->getSimAccessString();
		region_maturity->setText(maturity);
	}
