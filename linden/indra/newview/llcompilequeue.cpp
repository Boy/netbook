...
struct LLScriptQueueData
{
	...
	LLUUID mTaskId;
	LLUUID mItemId;
	LLScriptQueueData(const LLUUID& q_id, const char* name, const LLUUID& task_id, const LLUUID& item_id) :
...
BOOL LLFloaterScriptQueue::start()
{
	...
	snprintf(buffer, sizeof(buffer), "Starting %s of %d items.", mStartString, n_objects); 		/* Flawfinder: ignore */
										  ...
										  void* user_data, S32 status, LLExtStat ext_status)
{
	...
	{
		LLScrollListCtrl* list = LLUICtrlFactory::getScrollListByName(queue, "queue output");
		list->addCommentText(buffer.c_str());
	}
	delete data;
}
									...
									const LLUUID& item_id)
{
	...
	if(!lscript_compile(filename, dst_filename,	err_filename,
						compile_to_mono, uuid_string, gAgent.isGodlike()))
	{
