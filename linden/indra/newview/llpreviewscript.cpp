...
void LLScriptEdCore::initMenu()
{
	...
	menuItem = LLUICtrlFactory::getMenuItemCallByName(this, "SaveToFile");
	menuItem->setMenuCallback(onSaveToDiskMenu, this);
	menuItem->setEnabledCallback(enableSaveToDiskMenu);

	menuItem = LLUICtrlFactory::getMenuItemCallByName(this, "LoadFromFile");
	menuItem->setMenuCallback(onLoadFromDiskMenu, this);
	menuItem->setEnabledCallback(enableLoadFromDiskMenu);
...
// static 
void LLScriptEdCore::onSaveToDiskMenu(void* userdata)
{
	LLScriptEdCore* self = (LLScriptEdCore*)userdata;
	if (!self || !self->mEditor) return;
	self->mEditor->saveToFile();
}

// static 
void LLScriptEdCore::onLoadFromDiskMenu(void* userdata)
{
	LLScriptEdCore* self = (LLScriptEdCore*)userdata;
	if (!self || !self->mEditor) return;
	self->mEditor->loadFromFile();
}
...
// static 
BOOL LLScriptEdCore::enableSaveToDiskMenu(void* userdata)
{
	LLScriptEdCore* self = (LLScriptEdCore*)userdata;
	if (!self || !self->mEditor) return FALSE;
	return self->mEditor->canSaveToFile();
}

// static 
BOOL LLScriptEdCore::enableLoadFromDiskMenu(void* userdata)
{
	LLScriptEdCore* self = (LLScriptEdCore*)userdata;
	if (!self || !self->mEditor) return FALSE;
	return self->mEditor->canLoadFromFile();
}
	...
	mIsModifiable(FALSE)
{
	...
	mMonoCheckbox =	LLUICtrlFactory::getCheckBoxByName(this, "mono");
...
//MK
	if (RRenabled && object)
	{
		BOOL locked = !gAgent.mRRInterface.isAllowed (object->getRootEdit()->getID(), "detach");
		if (!locked)
		{
			...
			if (avatarp)
			{
				...
				locked = gAgent.mRRInterface.contains("detach:"+avatarp->getAttachedPointName(inv_item_id));
			}
		}
		if (locked)
		{
...
///MK
	if (RRenabled)
	{
		...
		if (object)
		{
			locked = !gAgent.mRRInterface.isAllowed (object->getRootEdit()->getID(), "detach");
			if (!locked)
			{
				LLVOAvatar *avatarp = gAgent.getAvatarObject();
				if (avatarp)
...
void LLLiveLSLEditor::processScriptRunningReply(LLMessageSystem* msg, void**)
{
	...
	if(LLLiveLSLEditor::sInstances.checkData(xored_id))
	{
		...
		LLCheckBoxCtrl* monoCheckbox = LLUICtrlFactory::getCheckBoxByName(instance, "mono");
