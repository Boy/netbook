...
void LLViewerPartGroup::updateParticles(const F32 lastdt)
{
	...
	for (i = 0; i < end; i++)
	{
		...
		if (part.mFlags & LLPartData::LL_PART_INTERP_COLOR_MASK)
		{
			...
			part.mColor %= 1.f - frac;
			part.mColor += frac%(frac*part.mEndColor); 
		}
...
void LLViewerPartSim::updateSimulation()
{
	...
	for (i = 0; i < count; i++)
	{
		...
		// invisible (culled or occluded) particle groups only get an update every 8 frames
		if ((LLDrawable::getCurrentFrame()+mViewerPartGroups[i]->mID)%visirate == 0)
		{
			...
			mViewerPartGroups[i]->mSkippedTime=0.f;
	...
	{
		if (sParticleCount > sMaxParticleCount * 0.875f && sParticleAdaptiveRate < 2.f)
		{
			sParticleAdaptiveRate *= 2.f;
		}
		else
		{
			if (sParticleCount < sMaxParticleCount * 0.5f && sParticleAdaptiveRate > 0.03125f)
			{
				sParticleAdaptiveRate *= 0.5f;
			}
		}
	}
