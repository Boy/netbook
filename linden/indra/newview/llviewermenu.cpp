...
void init_debug_rendering_menu(LLMenuGL* menu)
{
	...
	sub_menu->append(new LLMenuItemCheckGL("Palletized Textures",
											&LLPipeline::toggleRenderDebugFeature, NULL,
											&LLPipeline::toggleRenderDebugFeatureControl,
											(void*)LLPipeline::RENDER_DEBUG_FEATURE_PALETTE, KEY_F7, MASK_SHIFT|MASK_CONTROL));
...
//			&& gSelectMgr->getSelection()->getSelectType() != SELECT_TYPE_HUD
			&& !gSelectMgr->getSelection()->getFirstObject()->isHUDAttachment())
		{
			LLVector3 pos = gSelectMgr->getSelection()->getFirstObject()->getPositionRegion ();
			pos -= gAgent.getPositionAgent ();
			if (pos.magVec () >= 1.5)
			{
				return false;
...
{
//MK
	if (RRenabled && gAgent.forwardGrabbed())
	{
...
//mk
	{
		...
		gAgent.teleportViaLocation(gLastHitPosGlobal + hips_offset);
	}
	else
	{
...
class LLObjectEnableExport : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		if(new_value)
		{
			...
			if(gSelectMgr->getSelection()->applyToNodes(the_ff,false))
			{
...
//MK
	...
	if (RRenabled && gAgent.mRRInterface.mContainsUnsit && avatar &&  avatar->mIsSitting)
	{
		if (gAgent.getAvatarObject() &&
...
//MK
		...
		{
			LLVector3 pos = gViewerWindow->lastNonFloraObjectHit ()->getPositionRegion () + offset_single;
			pos -= gAgent.getPositionAgent ();
			if (pos.magVec () >= 1.5)
			{
				return true;
...
//MK
		if (RRenabled && gAgent.mRRInterface.mContainsRez
			&& gEditMenuHandler == gSelectMgr) // the Delete key must not be inhibited for text
		{
			new_value = false;
		}
//mk
			...
			if (RRenabled)
			{
				...
				if (attachment_point)
				{
					...
					if (!uuid.isNull() && (!gAgent.mRRInterface.isAllowed (uuid, "detach") || 
								(avatarp && !gAgent.mRRInterface.contains("detach:"+avatarp->getAttachedPointName(uuid)))))
					{
						setObjectSelection (NULL);
						return false;
					}
...
//MK
		if (RRenabled)
		{
			if (!gAgent.mRRInterface.isAllowed (attached_object->getID(), "detach"))
			{
				return;
			}

			LLVOAvatar *avatarp = gAgent.getAvatarObject();
			if (avatarp && gAgent.mRRInterface.contains("detach:"+gAgent.mRRInterface.findAttachmentNameFromPoint(attachment)))
			{
				return;
			}
		}
...
//MK
		if (RRenabled)
		{
			if (!gAgent.mRRInterface.isAllowed (object->getID(), "detach"))
			{
				return true;
			}
...
//MK
	if (RRenabled)
	{
		...
		if (gAgent.mRRInterface.mContainsDetach && gSelectMgr->getSelection()->getRootObjectCount() > 1)
		{
			return FALSE;
		}
	}
...
{
	QueueObjects func(q);
	LLObjectSelectionHandle selectHandle = gSelectMgr->getSelection();
...
//MK
	if (RRenabled && gAgent.mRRInterface.mContainsSetenv) {
		LLString setting = LLString(static_cast<char*>(user_data));
	        if (setting == "SkyOverrideSimSunPosition"
			|| setting == "MouseSun"
			|| setting == "FixedWeather")
        	{
                	checked = TRUE;
        	}
	}
...
//MK
			if (RRenabled && gAgent.mRRInterface.mContainsEdit) new_value = false;
//mk
