...
void LLGroupMgr::processGroupMembersReply(LLMessageSystem* msg, void** data)
{
	...
	if (group_datap->mMemberCount > 0)
	{
		...
		for (S32 i = 0; i < num_members; i++)
		{
			...
			{
				std::string last_on(online_status);
				tm t;
				if (sscanf(online_status, "%u/%u/%u", &t.tm_mon, &t.tm_mday, &t.tm_year) == 3 && t.tm_year > 1900)
				{
					t.tm_year -= 1900;
					t.tm_mon--;
					t.tm_hour = t.tm_min = t.tm_sec = 0;					
					timeStructToFormattedString(&t, gSavedSettings.getString("ShortDateFormat"), last_on);
				}
