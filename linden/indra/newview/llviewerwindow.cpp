...
void LLViewerWindow::reshape(S32 width, S32 height)
{
	...
	if (!LLApp::isExiting())
	{
		...
		// changes in window's width and height
...
BOOL LLViewerWindow::handlePerFrameHover()
{
	...
	// Show joints alt key is held down.
	if (gHUDManager)
	{
...
BOOL LLViewerWindow::saveImageNumbered(LLImageFormatted *image, const LLString& extension_in)
{
	if (!image)
	{
		return FALSE;
