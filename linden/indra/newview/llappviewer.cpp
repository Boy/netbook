...
const char*			DEFAULT_SETTINGS_FILE = "settings_1.19.xml";
...
const char* MARKER_FILE_NAME = "RainbowViewer.exec_marker";
const char* ERROR_MARKER_FILE_NAME = "RainbowViewer.error_marker";
const char* LLERROR_MARKER_FILE_NAME = "RainbowViewer.llerror_marker";
...
#ifdef LL_WINDOWS
	static char sWindowClass[] = "Rainbow Viewer";
#endif
...
//MK
					...
					&& !gFocusMgr.focusLocked())
				{
					...
					if (gAgent.mRRInterface.sAssetToReattach.isNull())
					{
...
bool LLAppViewer::initEarlyConfiguration()
{
	...
	for (j = 1; j < argc; j++) 
	{
		...
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_OSGRID].mName);
		}
		else if (!strcmp(argv[j], "--deepgrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_DEEPGRID].mName);
		}
		else if (!strcmp(argv[j], "--emerald"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_EMERALD].mName);
		}
		else if (!strcmp(argv[j], "--francogrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_FRANCOGRID].mName);
		}
		else if (!strcmp(argv[j], "--darwin"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_MONDEDARWIN].mName);
		}
		else if (!strcmp(argv[j], "--eugrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_EUGRID].mName);
		}
		else if (!strcmp(argv[j], "--giantgrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_GIANTGRID].mName);
		}
		else if (!strcmp(argv[j], "--newworld"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_NEWWORLD].mName);
		}
		else if (!strcmp(argv[j], "--3rdrock"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_3RDROCK].mName);
		}
		else if (!strcmp(argv[j], "--legendcity"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_LEGENDCITY].mName);
		}
		else if (!strcmp(argv[j], "--uvatar"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_UVATAR].mName);
		}
		else if (!strcmp(argv[j], "--gridsplash"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_GRIDSPLASH].mName);
		}
		else if (!strcmp(argv[j], "--worldsimterra"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_WORLDSIMTERRA].mName);
		}
		else if (!strcmp(argv[j], "--cyberlandia"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_CYBERLANDIA].mName);
		}
		else if (!strcmp(argv[j], "--tribalnet"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_TRIBALNET].mName);
		}
		else if (!strcmp(argv[j], "--metropolis"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_METROPOLIS].mName);
		}
		else if (!strcmp(argv[j], "--augrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_AUGRID].mName);
		}
		else if (!strcmp(argv[j], "--hangout"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_AVATARHANGOUT].mName);
		}
		else if (!strcmp(argv[j], "--germangrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_GERMANGRID].mName);
		}
		else if (!strcmp(argv[j], "--dgpgrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_DGPGRID].mName);
		}
		else if (!strcmp(argv[j], "--lifesim"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_LIFESIM].mName);
		}
		else if (!strcmp(argv[j], "--fantasyworld"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_FANTASYWORLD].mName);
		}
		else if (!strcmp(argv[j], "--rmhdesign"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_RMHDESIGN].mName);
		}
		else if (!strcmp(argv[j], "--reactiongrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_REACTIONGRID].mName);
		}
		else if (!strcmp(argv[j], "--futurelife"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_FUTURELIFE].mName);
		}
		else if (!strcmp(argv[j], "--pseudospace"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_PSEUDOSPACE].mName);
		}
		else if (!strcmp(argv[j], "--yourgrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_YOURGRID].mName);
		}
		else if (!strcmp(argv[j], "--myopengrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_MYOPENGRID].mName);
		}
		else if (!strcmp(argv[j], "--phoenixsim"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_PHOENIXSIM].mName);
		}
		else if (!strcmp(argv[j], "--virtualsims"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_VIRTUALSIMS].mName);
		}
		else if (!strcmp(argv[j], "--grid4us"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_GRID4US].mName);
		}
		else if (!strcmp(argv[j], "--openkansai"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_OPENKANSAI].mName);
		}
		else if (!strcmp(argv[j], "--schweiz"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_SCHWEIZ].mName);
		}
		else if (!strcmp(argv[j], "--dreamland"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_DREAMLAND].mName);
		}
		else if (!strcmp(argv[j], "--altlife"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_ALTLIFE].mName);
		}
		else if (!strcmp(argv[j], "--sciencesim"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_SCIENCESIM].mName);
		}
		else if (!strcmp(argv[j], "--unica"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_UNICA].mName);
		}
		else if (!strcmp(argv[j], "--gorgrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_GORGRID].mName);
		}
		else if (!strcmp(argv[j], "--cuongrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_CUONGRID].mName);
		}
		else if (!strcmp(argv[j], "--artgrid"))
		{
			sprintf(gGridName,"%s", gGridInfo[GRID_INFO_ARTGRID].mName);
		}
		else if (!strcmp(argv[j], "-user") && (++j < argc))
		{
			if (!strcmp(argv[j], "-"))
...
bool LLAppViewer::initConfiguration()
{
	...
	if (! gDirUtilp->getSkinDir().empty() )
	{
		...
		// Legacy string.  This flag really meant we didn't want to expose references to "Rainbow Viewer".
...
bool LLAppViewer::initWindow()
{
	...
	gViewerWindow = new LLViewerWindow(window_title_str, "Rainbow Viewer",
...
{
    return (GRID_INFO_ADITI != gGridChoice);
}
