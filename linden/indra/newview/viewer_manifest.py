...
class WindowsManifest(ViewerManifest):
        ...
        def construct(self):
                ...
                self.path(self.find_existing_file('ReleaseForDownload/RainbowViewer.exe', 'RainbowViewer.exe', 'ReleaseNoOpt/newview_noopt.exe'), dst=self.final_exe())
...
class DarwinManifest(ViewerManifest):
        def construct(self):
                ...
                self.path("build/" + self.args['configuration'] + "/Rainbow Viewer.app", dst="")
...
class LinuxManifest(ViewerManifest):
        def construct(self):
                ...
                self.path("res/ll_icon.ico","rainbowviewer.ico")
        ...
        def package_finish(self):
                if(self.args.has_key('installer_name')):
                        ...
                        installer_name = '_'.join('RainbowViewer_', self.args.get('arch'), *self.args['version'])
                        if self.default_channel():
                                if not self.default_grid():
...
class Linux_i686Manifest(LinuxManifest):
        def construct(self):
                ...
                self.path("rainbowviewer-i686-bin-stripped","bin/do-not-directly-run-rainbowviewer-bin")
...
class Linux_x86_64Manifest(LinuxManifest):
        def construct(self):
                ...
                self.path("rainbowviewer-x86_64-bin-stripped","bin/do-not-directly-run-rainbowviewer-bin")
