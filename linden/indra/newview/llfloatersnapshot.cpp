...
public:
	...
	ESnapshotFormat getSnapshotFormat() const { return mSnapshotFormat; }
	...
	void setSnapshotFormat(ESnapshotFormat type) { mSnapshotFormat = type; }
...
protected:
	...
	LLPointer<LLImageJPEG>		mFormattedJPEG;
	LLPointer<LLImagePNG>		mFormattedPNG;
	LLPointer<LLImageBMP>		mFormattedBMP;
	...
	ESnapshotFormat				mSnapshotFormat;
...
LLSnapshotLivePreview::LLSnapshotLivePreview (const LLRect& rect) : 
	...
	mSnapshotFormat(ESnapshotFormat(gSavedSettings.getS32("SnapshotFormat"))),
	...
	if (previewp->mSnapshotActive)
	{
								...
								previewp->mSnapshotBufferType))
		{
			...
			if (previewp->getSnapshotType() == SNAPSHOT_POSTCARD || 
				(previewp->getSnapshotType() == SNAPSHOT_LOCAL && previewp->getSnapshotFormat() == SNAPSHOT_FORMAT_JPEG))
			{
				// *FIX: just resize and reuse existing jpeg?
				previewp->mFormattedJPEG = NULL; // deletes image
				previewp->mFormattedJPEG = new LLImageJPEG();
				previewp->mFormattedJPEG->setEncodeQuality(llclamp(previewp->mSnapshotQuality, 0, 100));
				if (previewp->mFormattedJPEG->encode(previewp->mRawImage))
				{
					previewp->mDataSize = previewp->mFormattedJPEG->getDataSize();
					previewp->mFormattedJPEG->decode(previewp->mRawImageEncoded);
				}
			}
			else if (previewp->getSnapshotType() == SNAPSHOT_LOCAL && previewp->getSnapshotFormat() == SNAPSHOT_FORMAT_PNG)
			{
				previewp->mFormattedPNG = NULL;
				previewp->mFormattedPNG = new LLImagePNG();
				if (previewp->mFormattedPNG->encode(previewp->mRawImage))
				{
					previewp->mDataSize = previewp->mFormattedPNG->getDataSize();

					//either one of these will work, decode feels faster, go figure
					//previewp->mRawImageEncoded->copy(previewp->mRawImage);
					previewp->mFormattedPNG->decode(previewp->mRawImageEncoded);
				}
			}
			...
			else if (previewp->getSnapshotType() == SNAPSHOT_LOCAL && previewp->getSnapshotFormat() == SNAPSHOT_FORMAT_BMP)
			{
				previewp->mFormattedBMP = NULL;
                                previewp->mFormattedBMP = new LLImageBMP();
				if (previewp->mFormattedBMP->encode(previewp->mRawImage))
				{
					previewp->mDataSize = previewp->mFormattedBMP->getDataSize();
					previewp->mRawImageEncoded->copy(previewp->mRawImage);
					//previewp->mFormattedBMP->decode(previewp->mRawImageEncoded);
				}
			}
			else
			{
				llwarns << "Error generating snapshot preview" << llendl;
			}
...
LLFloaterPostcard* LLSnapshotLivePreview::savePostcard()
{
	...
	LLFloaterPostcard* floater = LLFloaterPostcard::showFromSnapshot(mFormattedJPEG, mViewerImage[mCurImageIndex], image_scale, mPosTakenGlobal);
...
{
	//saveImageNumbered works on a per-extension basis
	LLString extension;
	BOOL success;
	switch(getSnapshotFormat())
    {
	  case SNAPSHOT_FORMAT_JPEG: 
		extension = ".jpeg";
		success = gViewerWindow->saveImageNumbered(mFormattedJPEG, extension);
		break;
	  case SNAPSHOT_FORMAT_PNG: 
		extension = ".png";
		success = gViewerWindow->saveImageNumbered(mFormattedPNG, extension);
		break;
	  case SNAPSHOT_FORMAT_BMP: 
		extension = ".bmp";
		success = gViewerWindow->saveImageNumbered(mFormattedBMP, extension);
		break;
	  default: 
...
// static
ESnapshotFormat LLFloaterSnapshot::Impl::getFormatIndex(LLFloaterSnapshot* floater)
{
...
void LLFloaterSnapshot::Impl::updateControls(LLFloaterSnapshot* floater)
{
	...
	combo = LLUICtrlFactory::getComboBoxByName(floater, "local_format_combo");
	if (combo) combo->selectNthItem(gSavedSettings.getS32("SnapshotFormat"));
	
	ESnapshotFormat shot_format = getFormatIndex(floater);
...
//mk
	switch(shot_type)
	{
	  case LLSnapshotLivePreview::SNAPSHOT_POSTCARD:
		...
		floater->childSetEnabled("local_format_combo", FALSE);
	  ...
	  case LLSnapshotLivePreview::SNAPSHOT_LOCAL:
		...
		if (shot_format == SNAPSHOT_FORMAT_JPEG)
			floater->childSetEnabled("image_quality_slider", TRUE);
		else
			floater->childSetEnabled("image_quality_slider", FALSE);
		floater->childSetEnabled("local_format_combo", TRUE);
...
void LLFloaterSnapshot::Impl::onCommitSnapshotFormat(LLUICtrl* ctrl, void* data)
{
	...
	if (view)
	{
		S32 format=getFormatIndex(view);
		gSavedSettings.setS32("SnapshotFormat", format);
		if (format == SNAPSHOT_FORMAT_JPEG)
			view->childSetEnabled("image_quality_slider", TRUE);
		else
			view->childSetEnabled("image_quality_slider", FALSE);
		getPreviewView(view)->updateSnapshot(TRUE);
		updateControls(view);
	}
}
