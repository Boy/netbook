									   ...
									   void* user_data, S32 status, LLExtStat ext_status)
{
	...
	else
	{
			...
			LL_ERR_FILE_EMPTY == status)
		{
			...
			if (itemp) {
				llwarns << "Problem loading gesture: " << itemp->getName() << llendl;

				LLString warn_msg = "Gesture is missing from database!\n \n";

				warn_msg += 
						"\n \n"
						"The gesture named '" + itemp->getName() +"' could not be found.\n\n"
						"Please go to the inventory, search for the gesture and try to\n" 
						"open, play, activate or delete it.";

				LLString::format_map_t args;
				args["[MESSAGE]"] = warn_msg;
				gViewerWindow->alertXml("GenericAlert", args);				
			}
			// If there is no corresponding item in the inventory, chances are
			// this is a false error... Let's comment it out for now, till LL
			// fixes the problem on the asset servers side.
//			else
//			{
