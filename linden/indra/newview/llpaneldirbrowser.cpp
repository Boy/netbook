...
void LLPanelDirBrowser::updateMaturityCheckbox()
{
	...
	std::string control_name_pg = LLViewerUICtrlFactory::getCheckBoxByName(this, "incpg")->getControlName();
	std::string control_name_mature = LLViewerUICtrlFactory::getCheckBoxByName(this, "incmature")->getControlName();
	std::string control_name_adult = LLViewerUICtrlFactory::getCheckBoxByName(this, "incadult")->getControlName();
...
//		msg->getString("QueryReplies", "Date", MAX_STRING, date, i);
		...
		{
			image_id.set( gViewerArt.getString("icon_event_mature.tga") ); // icon_event_adult.tga
			row["columns"][0]["column"] = "icon";
			row["columns"][0]["type"] = "icon";
			row["columns"][0]["value"] = image_id;
		}
		...
		row["columns"][2]["value"] = date.c_str();
		row["columns"][2]["font"] = "SANSSERIFSMALL";
