...
void LLViewerPartSourceScript::update(const F32 dt)
{
	...
	F32 ref_rate_greedlimit = llmax(gWorldPointer->mPartSim.getRefRate(), 0.25f);
	...
	while ((dt_update > mPartSysData.mBurstRate) || first_run)
	{
		...
		F32 max_short_side = llmax(
								llmax(llmin(mPartSysData.mPartData.mStartScale[0], mPartSysData.mPartData.mStartScale[1]),
									  llmin(mPartSysData.mPartData.mEndScale[0], mPartSysData.mPartData.mEndScale[1])),
								llmin((mPartSysData.mPartData.mStartScale[0] + mPartSysData.mPartData.mEndScale[0])/2,
									  (mPartSysData.mPartData.mStartScale[1] + mPartSysData.mPartData.mEndScale[1])/2));
		...
		if (max_dist < 0.25f)
		{
			// Particles this tiny are useless and mostly spawned by buggy sources
			mLastPartTime = mLastUpdateTime;
			break;
		}
		...
		if (ref_rate_greedlimit >= 0.25f)
		{
			if (mPartSysData.mBurstPartCount * (mPartSysData.mPartData.mMaxAge / mPartSysData.mBurstRate ) * ref_rate_greedlimit > 256.f)
			{
				limited_rate = llmax(limited_rate,(1-256.f/(mPartSysData.mBurstPartCount * (mPartSysData.mPartData.mMaxAge / mPartSysData.mBurstRate ) * ref_rate_greedlimit)));
			}
		}
		...
		{
			if (ll_frand() < llmax (gWorldPointer->mPartSim.maxRate(),limited_rate))
			{
