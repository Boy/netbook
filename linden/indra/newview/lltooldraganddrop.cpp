...
//MK
		if (RRenabled)
		{
			...
			if (!gAgent.mRRInterface.isAllowed (hit_obj->getRootEdit()->getID(), "detach"))
			{
				return;
			}
			...
			LLInventoryItem* inv_item = gAgent.mRRInterface.getItem(hit_obj->getRootEdit()->getID());
...
//MK
	if (RRenabled)
	{
		...
		if (!gAgent.mRRInterface.isAllowed (obj->getRootEdit()->getID(), "detach"))
		{
			return ACCEPT_NO_LOCKED;
		}
		...
		LLVOAvatar *avatarp = gAgent.getAvatarObject();
		LLUUID inv_item_id = LLUUID::null;
		LLInventoryItem* inv_item = gAgent.mRRInterface.getItem(obj->getRootEdit()->getID());
		if (inv_item) inv_item_id = inv_item->getUUID();
		if (avatarp && gAgent.mRRInterface.contains("detach:"+avatarp->getAttachedPointName(inv_item_id)))
		{
			return ACCEPT_NO_LOCKED;
