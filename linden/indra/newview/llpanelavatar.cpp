...
// 	if (avatar_changed)
	{
		...
		LLNameEditor* avatar_key = LLViewerUICtrlFactory::getNameEditorByName(this, "avatar_key");
		if (avatar_key)
		{
...
void LLPanelAvatar::processAvatarPropertiesReply(LLMessageSystem *msg, void**)
{
	...
	for (panel_list_t::iterator iter = sAllPanels.begin(); iter != sAllPanels.end(); ++iter)
	{
		...
		std::string born(born_on);
		tm t;
		if (sscanf(born_on, "%u/%u/%u", &t.tm_mon, &t.tm_mday, &t.tm_year) == 3 && t.tm_year > 1900)
		{
			t.tm_year -= 1900;
			t.tm_mon--;
			t.tm_hour = t.tm_min = t.tm_sec = 0;
			timeStructToFormattedString(&t, gSavedSettings.getString("ShortDateFormat"), born);
		}
		...
		self->mPanelSecondLife->childSetValue("born", born.c_str());
