...
//MK
		else if (RRenabled && !strcmp(message, "@version"))
		{
			// return the version message
			std::string my_name;
			gAgent.buildFullname(my_name);
			LLString response = gAgent.mRRInterface.getVersion ();
			pack_instant_message(
				gMessageSystem,
				gAgent.getID(),
				FALSE,
...
//MK
			if (RRenabled && gAgent.mRRInterface.contains ("recvim"))
			{
				...
				if (!gAgent.mRRInterface.contains ("recvim:"+from_id.asString ()))
				{
					...
					snprintf(buffer, sizeof(buffer), "%s%s*** IM blocked by your viewer", separator_string, saved);
					
					// tell the sender the avatar could not read them
					std::string my_name;
					gAgent.buildFullname(my_name);
					my_name = my_name+" using viewer "+gAgent.mRRInterface.getVersion ();
					LLString response = "The Resident you messaged is prevented from reading your instant messages at the moment, please try again later.";
					pack_instant_message(
						gMessageSystem,
						gAgent.getID(),
						FALSE,
...
//MK
			...
			{
				std::string str(message);
				str = gAgent.mRRInterface.getCensoredMessage(message);
				strcpy (message, str.c_str());		
			}
//mk
			args["[MESSAGE]"] = message;
...
//MK
				...
				{
					LLString response = "The Resident you invited is prevented from accepting teleport offers. Please try again later.";
					pack_instant_message(
						gMessageSystem,
						gAgent.getID(),
						FALSE,
...
//MK
		if (RRenabled && chat.mChatType != CHAT_TYPE_OWNER)
		{
			...
			{
				if (gAgent.mRRInterface.contains ("recvchat") &&		// user is forbiddden to hear chat ...
					!gAgent.mRRInterface.contains ("recvchat:"+from_id.asString()))	// ... and sender is no exception
				{
					chat.mFromName = from_name;				
					chat.mText = mesg;
					chat.mText = gAgent.mRRInterface.crunchEmote (chat.mText, 20);
					strncpy(mesg, chat.mText.c_str(), DB_CHAT_MSG_BUF_SIZE);
					mesg[DB_CHAT_MSG_BUF_SIZE - 1] = '\0';
				}

				if (gAgent.mRRInterface.contains ("recvemote") &&		// user is forbiddden to see emotes ...
					!gAgent.mRRInterface.contains ("recvemote:"+from_id.asString()))	// ... and sender is no exception
				{
					if (strncmp(mesg, "/me ", 4) == 0 || strncmp(mesg, "/me'", 4) == 0)
					{
						chat.mFromName = from_name;				
						chat.mText = "/me ...";
						strcpy(mesg, chat.mText.c_str());
					}
				}

				if (!is_self && gAgent.mRRInterface.mContainsShownames)
				{
					// also scramble the name of the chatter (replace with a dummy name)
					if (chatter->isAvatar ())
					{
						std::string from_name_as_string (from_name);
						strcpy (from_name,
							gAgent.mRRInterface.getDummyName (
								from_name_as_string, chat.mAudible
							).c_str());
					}
					else
					{
						std::string str (from_name);
						str = gAgent.mRRInterface.getCensoredMessage (from_name);
						strcpy (from_name, str.c_str());
					}
					chat.mFromName = from_name;				
				}
			}
...
//MK
			// This is the actual handling of the commands sent by owned objects.
			{
				...
				{
					LLString command = mesg_str.substr(1, mesg_str.length());
...
//				}
				else
				{
					...
					{
						// hide every occurrence of the Parcel name if the location restriction is active
						str = gAgent.mRRInterface.stringReplace (str, 
					...
					if (RRenabled && gAgent.mRRInterface.mContainsShownames)
					{
						...
						from = gAgent.mRRInterface.getCensoredMessage (from);
						strcpy (from_name, from.c_str());
						chat.mFromName = from_name;				
					}

					strcpy (mesg, str.c_str());		
					verb = ": ";
				}
				break;
			}
...
char* formatted_time(const time_t& the_time, char* buffer)
{
	...
	LLString::copy(buffer, timestr.c_str(), TIME_STR_LENGTH);
	buffer[24] = '\0';
	return buffer;
}
